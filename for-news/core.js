...
module.exports = {
    getPageData: function (req, res) {
        ...
        if (req.query.host == 'news.zhibo8.cc') {
            let splits = req.query.path.split('/')
        ...
        } else if (req.query.host == 'www.zhibo8.cc') {
            let splits = req.query.path.split('/')
            //0816-fangshou
            let tailUrl = splits[splits.length - 1].split('.')[0];
        ...
        /*
        if (req.query.isPage) {
            getRequestData(req.query.host, req.query.path).then(function (data) {
                try {
        ...
        */
        ...
        Promise.all([getRequestData(req.query.host, req.query.path), getRequestData('cache.zhibo8.cc', hotCommentPath), getRequestData('cache.zhibo8.cc', commentPath)])
            .then(function (resps) {
                var hotCommentData;
                var commentData;
                var htmlData;
                if (resps.length == 3) {
                    for (let resp of resps) {
                        switch (resp.path) {
                            case req.query.path: htmlData = resp.data
                                continue
    ...
    getIncrementalData: function (req, res) {
        var maxDateList = req.query.maxDateList;
        getRequestData('m.zhibo8.cc', '/json/hot/24hours.htm').then(function (respData) {
            try {
                let data = util.parseJson(respData.data)
    ...
    getMoreData: function (req, res) {
        var currentMinDate = new Date(req.query.currentMinDate)
        var intervalDay = parseInt(req.query.intervalDay)
    ...
    getLivePageData: function (req, res) {
        ...
        Promise.all([getRequestData(req.query.host, req.query.path), getRequestData('cache.zhibo8.cc', commentPath)])
            .then(function (data) {
                var commentData
                var htmlData
                if (data.length == 2) {
                    if (data[0].path == req.query.path) {
